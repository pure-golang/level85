...
- `State`, `GetOrCreateStack`, `GeneratePrefix`, common Given-шаги живут в `shared` и экспортированы; приватные хелперы конкретного эпика остаются файл-local в `shared/steps_NN_<name>.go`.
