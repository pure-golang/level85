...
Он не владеет layout/naming `.feature` и не владеет dev RGB-циклом. За это отвечают `x-bdd-api` и `x-bdd-dev-workflow`.
...
- результат сохраняется в `features/NN_epic/NN_story.feature`
- в monorepo проставляй execution tags `@api`, `@browser`, позже `@mobile`
- в backend-only/frontend-only execution tags не добавляй без отдельной причины
- если тип проекта или канал исполнения неясен, фиксируй вопрос, а не выбирай `@api` по умолчанию

### 4. Сверяй результат с BDD-конвенциями

- naming, numbering и структура файла проверяются по `x-bdd-api`
...
- итоговый `.feature` готов к проверке по `x-bdd-api`
