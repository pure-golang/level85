...
description: "Применяй вместе с `skill-creator`, когда создаёшь или редактируешь локальный skill в `level85`: project-specific требования к `SKILL.md`, `scripts/`, `references/`, `assets/`, аудируемости и запрету ссылок на эталонные репозитории"
---
...
- решаешь, что оставить в `SKILL.md`, а что вынести в `scripts/`, `references/` или `assets/`
...
- `SKILL.md` держит условия срабатывания, workflow, правила выбора и границы применения
- `references/` держит детали, варианты, самопроверку и примеры, которые перегружают основной текст
...
Не дублируй одно и то же между `SKILL.md` и `references/`.
...
- если важен сам шаблон результата, предпочитай `assets/` или `references/`, а не генераторный script
...
- для skill scripts предпочитай размер порядка `~100` строк или меньше; если script заметно растёт, это сигнал вынести логику обратно в `SKILL.md`, `references/` или `assets/`
...
- детали действительно вынесены в `references/`, а не продублированы
