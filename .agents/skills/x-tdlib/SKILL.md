...
Smoke-тесты проверяют тонкий main, а не TDLib. Детали → `x-testing-conventions`.
...
Интерфейс `authDriver` абстрагирует UI авторизации (терминал, бот, и т.д.) от логики переходов.
...
- unit-тесты используют mockery-моки, не FakeTelegram
