...
- любыми godog step definitions в layout текущего репозитория;
...
- `x-bdd-godog` для layout/naming/wiring;
...
Для затронутого эпика посмотри `.feature` и соответствующие step definition файлы. Их расположение определяй по текущему репозиторию и `x-bdd-godog`; этот skill не задаёт layout.
