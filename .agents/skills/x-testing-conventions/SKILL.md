...
description: "Применяй при написании или правке любого теста в проекте: выбор слоя (`unit`/`features`/`api-bdd`/`browser-bdd`/`smoke`), `t.Parallel()`, build tags, AAA и базовая структура проверки"
---
# Testing Conventions

Этот skill — **канонический владелец** общих правил тестового слоя:
- layer selection и физическое расположение теста
- маркеры слоя (`t.Parallel()`, build tags, runner config)
- AAA-структура
...
| Слой | Где живёт | Маркер | Команда запуска |
|---|---|---|---|
| unit | `*_test.go` рядом с кодом | `t.Parallel()` по умолчанию | `task test-short` (или `task backend:test-short` для monorepo) |
| features | `features/NN_epic/NN_story.feature` | Gherkin, не runner | см. BDD-skills |
| api-bdd | monorepo: `backend/test/bdd/`; backend-only: `test/bdd/` | `//go:build bdd`; `@api` только в monorepo | `task test-bdd` (или `task backend:test-bdd` для monorepo) |
| browser-bdd | monorepo: `frontend/test/bdd/`; frontend-only: `test/bdd/` | `playwright-bdd`; `@browser` только в monorepo | `task test-bdd` (или `task frontend:test-bdd` для monorepo) |
| smoke | `test/smoke/` | `//go:build smoke` | `task test-smoke` (или `task backend:test-smoke` для monorepo) |

Если тест проверяет систему на языке бизнеса, смотри BDD-skills. `.feature` файлы не являются runner-слоем: они живут в `features/`, а реализация лежит в API или browser runner-е.

#### Как запускать локально

- `task test-short` (или `task backend:test-short` для monorepo) — быстрая петля разработки. Прогоняет unit и не должен поднимать BDD/smoke инфраструктуру.
- `task test-bdd` (или `task backend:test-bdd` для monorepo) — точечный запуск API BDD runner-а.
- Browser BDD запускается runner script из `x-bdd-browser`.
- `task test-smoke` (или `task backend:test-smoke` для monorepo) — точечный запуск smoke-слоя.

Маркер слоя должен соответствовать команде: если unit-тест случайно зависит от реального контейнера, `task test-short` начнёт требовать внешние сервисы, и быстрая петля сломается.
...
- api-bdd маркируется build tag `bdd`, а не через `t.Parallel()`
- browser-bdd маркируется runner config/script, а не через `t.Parallel()`
- smoke маркируется build tag `smoke`
...
Для api-bdd, browser-bdd и smoke `t.Parallel()` не является нормой слоя и обычно не нужен.
...
- api-bdd и browser-bdd тесты не нужно "ускорять" через `t.Parallel()`; их маркер слоя другой
- smoke тесты тоже не должны опираться на `t.Parallel()` как на соглашение
...
- тест с внешней инфраструктурой лежит рядом с unit-кодом без явного маркера слоя
...
- api-bdd/browser-bdd/smoke не используют `t.Parallel()` как маркер слоя
- `t.Skip()` не используется для сокрытия падающего теста
...
- testcontainers setup → `x-testcontainers-go`
...
- бизнес-сценарии (→ `features/` + BDD runner)
- контракты адаптеров (→ специализированный test layer проекта)
