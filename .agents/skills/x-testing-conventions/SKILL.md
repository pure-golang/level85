...
|---|---|---|---|
| unit | `*_test.go` рядом с кодом | `t.Parallel()` по умолчанию | `task test-short` |
| features | `features/NN_epic/NN_story.feature` | Gherkin, не runner | см. BDD-skills |
| api-bdd | monorepo: `backend/test/bdd/`; backend-only: `test/bdd/` | `//go:build bdd`; `@api` только в monorepo | `task test-bdd` |
| browser-bdd | monorepo: `frontend/test/bdd/`; frontend-only: `test/bdd/` | `playwright-bdd`; `@browser` только в monorepo | runner script |
| smoke | `test/smoke/` | `//go:build smoke` | `task test-smoke` |
...
- `task test-short` — быстрая петля разработки. Прогоняет unit и не должен поднимать BDD/smoke инфраструктуру.
- `task test-bdd` — точечный запуск API BDD runner-а.
- Browser BDD запускается runner script из `x-bdd-browser`.
- `task test-smoke` — точечный запуск smoke-слоя.
