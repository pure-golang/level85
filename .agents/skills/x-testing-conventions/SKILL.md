...
| bdd | `test/bdd/` | `//go:build bdd` | `task test-bdd` |
...
- `task test` — полный прогон с integration. Выполняется долго, требует docker/testcontainers. Запускай перед коммитом или когда менял адаптеры, репозитории.
- `task test-smoke`, `task test-e2e`, `task test-bdd` — точечный запуск конкретного слоя через build tags.
